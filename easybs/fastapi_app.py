...
PREVIEW_DIR = os.environ.get("PREVIEW_DIR", r".\3D_png")
os.makedirs(PREVIEW_DIR, exist_ok=True)
#%%
...
def plot_zone_mat():
    try:
        ...
        out_path = _plot_zone_mean_air_temp(csv_path, PREVIEW_DIR)
        url = "/api/static/" + os.path.basename(out_path)   # served by Node
...
def plot_rfh_heating_rate():
    try:
        csv_path = _find_eplus_csv()
        out_path = _plot_rfh_heating_rate(csv_path, PREVIEW_DIR, sum_all=True)
        url = "/api/static/" + os.path.basename(out_path)
        return {"ok": True, "url": url}
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
...
app.mount("/api/static", StaticFiles(directory=PREVIEW_DIR), name="api_static")

def _find_latest_png():
    files = glob.glob(os.path.join(PREVIEW_DIR, "*.png"))
    return max(files, key=os.path.getmtime) if files else None

@app.get("/api/preview/latest")
def preview_latest():
    latest = _find_latest_png()
    if not latest:
        raise HTTPException(status_code=404, detail="No preview image found.")
    # return a URL the browser can request
    return {"ok": True, "url": "/api/static/" + os.path.basename(latest)}
